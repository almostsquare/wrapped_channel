...
async def test_start(dut):
    ...
    await with_timeout(RisingEdge(dut.uut.mprj.wrapped_channel.active), 1500, 'us')

    # wait for channel module to come out of reset
    await with_timeout(FallingEdge(dut.uut.mprj.wrapped_channel.gps_channel0.reset), 1500, 'us')

    # wait for LO NCO to be turned on
    await with_timeout(RisingEdge(dut.uut.mprj.wrapped_channel.gps_channel0.lo_nco_enable), 3000, 'us')

    # wait for CA generator to be turned on
    await with_timeout(RisingEdge(dut.uut.mprj.wrapped_channel.gps_channel0.ca_gen_enable), 3000, 'us')
